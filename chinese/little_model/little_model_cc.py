...
def read(filename,val=False):
    ...
    with open(filename,encoding='gbk') as f:
        ...
        headers = next(f_csv)
        for row in f_csv:
            if str(row[4]).replace('(1)主诉：','')==str(row[3]) or str(row[4]).replace('(1)主诉：','')==str(row[2]) or str(row[4]).replace('(1)主诉：','') in str(row[3]).replace('(1)主诉：','') or str(row[4]).replace('(1)主诉：','') in str(row[2]).replace('(1)主诉：',''):
                continue
            reject_text=row[7]+'\n'+row[4]
            reject_text=reject_text.replace('(2)现病史：','')
            reject.append(reject_text)
            if len(row[2])>len(row[3]):
                accept_text=row[7]+'\n'+'(1)主诉：'+row[2]
                accept_text=accept_text.replace('(2)现病史：','')
                accept.append(accept_text)
            else:
                accept_text=row[7]+'\n'+'(1)主诉：'+row[3]
                accept_text=accept_text.replace('(2)现病史：','')
                accept.append(accept_text)
